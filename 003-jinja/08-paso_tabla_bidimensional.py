...
import pymysql                         # Importamos MySQL
###################### MYSQL ##############################################
conexion = pymysql.connect(
host="localhost",user="Bryan",password="AAAlog007.",database="portafolioceac"
...
cursor.execute("SHOW COLUMNS in Piezas;") # Muestra las tablas de la base de datos
columnas = []                             # Creo una lista vacia
filas = cursor.fetchall()                 # Lo guardo en una lista
for fila in filas:                        # Recorro el resultado
  columnas.append(fila[0])                # Lo añado a la lista de tablas 
# ----------------- ESTO ENVIA TODA LA TABLA ---------
cursor.execute("SELECT * FROM Piezas;")   # Muestra las tablas de la base de datos
